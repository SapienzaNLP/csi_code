    ...
    for sent_idx, sentence_word in enumerate(tokens):
        for token_idx, token in enumerate(sentence_word):
            if not token == 'PADDING' and not token == 'untagged' and not token is None:
                ...
                sense_indexes = [labels_dict[domain] for domain in domains if domain in labels_dict and not domain is None]
...
def buildOneOutDictionary(config):
    ...
    semcor_distribution_counts_csi = getSemcorDistribution(path_all_words_semcor, csi) # lemma--> num coarse senses in semcor
    semcor_distribution_counts_wnd = getSemcorDistribution(path_all_words_semcor, wnd) # lemma--> num coarse senses in semcor
    semcor_distribution_counts_lex = getSemcorDistribution(path_all_words_semcor, lex) # lemma--> num coarse senses in semcor
